...
process.stdin.on('end', async () => {
  try {
    const [syncParamsStatus, syncRowsStatus] = await Promise.all([syncParams(), syncParamsRows()]);
    ...
    // GEÇİCİ TEŞHİS — Blob senkronizasyonunun neden başarısız olduğunu görmek için.
    // Sorun bulunup düzeltilince kaldırılacak.
    result._debugSync = { params: syncParamsStatus, rows: syncRowsStatus };
