...
process.stdin.on('end', async () => {
  try {
    ...
    debug.tokenPresent = !!tok;
    debug.tokenLen = tok ? tok.length : 0;
    debug.tokenPrefix = tok ? tok.slice(0, 20) : null;
    debug.tokenHasNewline = tok ? /[\r\n]/.test(tok) : null;
    if (tok) {
      try {
        const info = await head('admin-store/params.json', { token: tok.trim() });
        debug.headOk = true;
