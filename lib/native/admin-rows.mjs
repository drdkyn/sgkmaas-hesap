...
const LIVE_ROWS_PATH = tmpPath('params-rows.json');
const ROWS_BLOB_KEY = 'admin-store/params-rows.json';
...
/** Vercel'de: Blob'dan güncel satırları /tmp'e çeker. Yerelde (Blob bağlı değilse) no-op. */
export async function syncParamsRows() {
  return hydrateFromBlob(ROWS_BLOB_KEY, LIVE_ROWS_PATH);
}

export function loadParamsRows() {
  try { return JSON.parse(fs.readFileSync(LIVE_ROWS_PATH, 'utf8')); } catch {}
  try { return JSON.parse(fs.readFileSync(ROWS_PATH, 'utf8')); } catch { return { yearly: {}, custom: [] }; }
}

export async function saveParamsRows(obj) {
  const json = JSON.stringify(obj, null, 2);
  try {
    fs.writeFileSync(ROWS_PATH, json, 'utf8'); // yerelde repo dosyasına yazar
  } catch {
    fs.writeFileSync(LIVE_ROWS_PATH, json, 'utf8'); // Vercel: repo salt-okunur, /tmp'e düş
  }
  await persistToBlob(ROWS_BLOB_KEY, obj); // kalıcı kaynak — Blob bağlı değilse no-op
}
...
export async function addYearlyRow(col, yil, deger) {
  const rows = loadParamsRows();
  if (!rows.yearly[col]) rows.yearly[col] = {};
  rows.yearly[col][String(yil)] = deger;
  await saveParamsRows(rows);
  return yearlyRefs(yil, col, deger);
}

export async function addCustomRow(grupId, label, refs) {
  const rows = loadParamsRows();
...
/** Yanlış eklenen bir yıllık satırı kaldırır; kaldırılan hücre referanslarını döner. */
export async function removeYearlyRow(col, yil) {
  const rows = loadParamsRows();
  const removedRefs = rows.yearly?.[col]?.[String(yil)] !== undefined ? yearlyRefs(yil, col, rows.yearly[col][String(yil)]) : null;
  if (rows.yearly?.[col]) delete rows.yearly[col][String(yil)];
  await saveParamsRows(rows);
  return removedRefs;
}

/** Yanlış eklenen bir serbest satırı (ref'ine göre) kaldırır; kaldırılan hücre referanslarını döner. */
export async function removeCustomRow(ref) {
  const rows = loadParamsRows();
  let removedRefs = null;
  rows.custom = (rows.custom || []).filter((item) => {
    if (item.refs && Object.prototype.hasOwnProperty.call(item.refs, ref)) {
      removedRefs = item.refs;
      return false;
    }
    return true;
  });
