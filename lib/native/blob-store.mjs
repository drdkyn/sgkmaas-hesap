...
export async function hydrateFromBlob(key, localPath) {
  ...
  try {
    ...
    // allowOverwrite ile aynı URL her yazımda yeniden kullanılıyor; Vercel'in CDN'i içeriği
    // "değişmez" varsayıp eski sürümü önbellekte tutabiliyor (doğrulandı: kayıt başarılı
    // oluyor ama hesaplama eski değeri okumaya devam ediyordu). uploadedAt'ı sorgu
    // parametresi olarak ekleyip her güncellemede farklı bir URL'den okumaya zorluyoruz.
