...
function createEngine(book, opts = {}) {
  ...
  function VLOOKUP(ctx, key, rangeRef, colIdx, approx) {
    ...
    if (exact) {
      for (let r = from.row; r <= to.row; r++) { const cv = getVal(sheet, r, from.col); if (eqExcel(cv, k)) return getVal(sheet, r, resCol); }
      return ERR.NA;
    }
  ...
  function reset() { cache.clear(); lastVal.clear(); overrides.clear(); inProgress.clear(); cyclic.clear(); stack = []; stats.evaluated = 0; stats.backEdges = 0; stats.errors = {}; stats.errEx = {}; }

  return { get, getVal, getByKey, solve, sweepCycles, setInput, reset, stats, cyclic, lastVal, book };
}
