...
export function hesapla(inputs = {}) {
  ...
  // Prim gün / çakışan: PÖGS = Giriş!N6 (SGK toplam). Çakışan İŞYERİ-DUYARLI (JS): aynı ay AYNI işyeri
  // >30 = ücret bölünmesi (toplama dahil, çakışma değil); FARKLI işyeri ay-aşımı = çakışan hizmet.
  const pogs = Number(eng.get('Giriş!N6')) || 0;
