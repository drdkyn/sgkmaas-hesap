...
const INPUT_MAP = {
  ...
  // NOT: işe giriş motor tarafından Hiz.Dökümü'nden türetilir (girilmez).
};
...
export function hesapla(inputs = {}) {
  ...
  // Prim gün / çakışan — JS'de PARSE SATIRLARINDAN hesaplanır. (Motorun Giriş!N6'sı 'Hizmet'
  // sayfasının gömülü iç-parse verisinden okur; biz Hiz.Dökümü'yü beslediğimiz için N6 stale kalır
  // → kullanılmaz.) Toplam PÖGS = net günler (Asıl/Ek +, İptal -) toplamı; çakışan işyeri-duyarlı.
  const cc = (Array.isArray(inputs.hizmetRows) && inputs.hizmetRows.length) ? cakismaCross(inputs.hizmetRows) : { cakisan: 0, detay: [] };
  let pogs = 0;
  if (Array.isArray(inputs.hizmetRows)) {
  ...
  result.primGunSayisi = pogs - cc.cakisan;   // net
