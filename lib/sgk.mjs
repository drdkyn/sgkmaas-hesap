...
export function statuKontrol(rows, tahsisSerial) {
  ...
  // Rakip statü: kişinin GERÇEKTEN hizmeti olan (toplam > 0) ikinci statü. Tek statülüyse yok.
  const ikinci = sorted.slice(1).find(s => s.toplam > 0) || null;
  const fark = sorted[0].belirleyici - (ikinci ? ikinci.belirleyici : 0);
  // İkinci statünün BELİRLEYİCİ olması için ulaşması gereken gün (eşik):
  //  - Son 7 yıl: pencere SABİT (2520, sıfır-toplam) -> kazanılan gün diğerinden düşer ->
  //    çoğunluk için yarıdan fazla: floor(pencere/2)+1.
  //  - Toplam: günler bağımsız -> kazananı geçmek için kazanan+1.
  const pencere = sonYedi ? acc : 0;
  const esik = sonYedi ? (Math.floor(pencere / 2) + 1) : (sorted[0].belirleyici + 1);
  const gerekenDaha = ikinci ? Math.max(0, esik - ikinci.belirleyici) : 0;
  return {
    belirlenen,
    ...
    statuler, fark, ikinciAd: ikinci ? ikinci.ad : null, esik, gerekenDaha,
  };
}
