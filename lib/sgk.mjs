...
export function statuKontrol(rows, tahsisSerial) {
  ...
  const dagilim = (ilkOnce && tahsis) ? win : total;
  let belirlenen = null, max = -Infinity;
  for (const [k, v] of Object.entries(dagilim)) if (v > max) { max = v; belirlenen = k; }
  return {
    belirlenen,
    kural: (ilkOnce && tahsis) ? 'Son 7 yıl hizmet dağılımı (01.10.2008 öncesi ilk giriş)' : 'En çok hizmet (01.10.2008 sonrası ilk giriş)',
