...
export function hesapla(inputs = {}) {
  ...
  if (Array.isArray(inputs.hizmetRows) && inputs.hizmetRows.length) {
    // PÖGS = Σ (dönem × işyeri) net günü, her grup ençok 30 (SGK: bir işyeri ayda en fazla 30 gün
    // bildirir → aynı işyerinin bölünmüş 31-günlük ayı 30'a kapanır). Farklı işyeri ay-aşımı PÖGS'te
    // kalır; emeklilik çakışması (cc) ayrıca düşer.
    const grp = {};
